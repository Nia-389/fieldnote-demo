  ...
  return (
    <main className="app-shell" onKeyDown={handleAppKey} tabIndex={-1}>
      ...
      <section className="workspace">
        ...
        {view === "export" && <ExportView exportCSVs={exportCSVs} onBackup={() => exportBackup()} importBackup={importBackup} fileInput={fileInput} />}
      </section>
      {newProjectOpen && <NewProjectDialog onClose={() => setNewProjectOpen(false)} onCreate={createBlankProject} />}
      {conflictingProject && <ConflictDialog project={conflictingProject} onResolve={resolveConflict} />}
      {toast && <div className="toast" role="status">✓ {toast}</div>}
...
function ExportView({ exportCSVs, onBackup, importBackup, fileInput }: { exportCSVs: () => void; onBackup: () => void; importBackup: (e: ChangeEvent<HTMLInputElement>) => void; fileInput: React.RefObject<HTMLInputElement | null> }) { return <div className="page"><div className="page-heading"><div><p className="kicker">PORTABILITY</p><h1>Export</h1><p>Keep a restorable project copy or take analysis-ready research data into other workflows.</p></div></div><div className="export-section-heading"><span className="eyebrow">PROJECT BACKUP</span><h2>Move or restore the complete workspace</h2></div><div className="export-grid"><section className="export-card"><span className="export-icon">{`{ }`}</span><h2>Download project backup</h2><p>Save all documents, codes, annotations, memos, settings, and progress in one versioned Fieldnote JSON file.</p><button className="primary-button" onClick={onBackup}>Download project file <span>↓</span></button></section><section className="export-card"><span className="export-icon">↥</span><h2>Open project file</h2><p>Add a Fieldnote project backup to this browser. Existing projects stay separate unless you explicitly replace one.</p><input ref={fileInput} type="file" accept="application/json,.json" onChange={importBackup} hidden /><button className="secondary-button" onClick={() => fileInput.current?.click()}>Choose project file</button></section></div><div className="export-section-heading research-heading"><span className="eyebrow">RESEARCH DATA</span><h2>Export analysis-ready tables</h2></div><section className="export-card featured research-export"><span className="export-icon">CSV</span><div><h2>Four linked CSV files</h2><p>Download documents, segments, annotations, and the codebook. Annotations use one segment-code pair per row.</p><ul><li>documents.csv</li><li>segments.csv</li><li>annotations.csv</li><li>codebook.csv</li></ul></div><button className="primary-button" onClick={exportCSVs}>Download 4 CSV files <span>↓</span></button></section><div className="privacy-note"><span>⌂</span><p><b>Your data stays local.</b><br />Nothing is uploaded to a server. Export a project backup before clearing browser data or moving devices.</p></div></div>; }
